---
title: 'Lab 5: Generalized Linear Regression'
author: "Hao Wang"
...
library(MASS)
library(effects)
library(nnet) #neural net package, which contains multinomial function
library(ggplot2)
library(Zelig)
library(ZeligChoice)
```
...
$$Y = \sigma(y) = F(x) = \frac{1}{1 + e^{-(\beta_0 + \beta_1*x_1 + \beta_2*x_2 ...)}}$$
...
```{r}
z.ologit <- zelig(poverty ~ country + religion + age + degree, data=mydata, model="ologit")
summary(z.ologit)
```
